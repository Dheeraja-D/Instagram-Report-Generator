...
def generate_annual_report(posts, profile, pdf_filename, start_date, end_date):
        ...
        try:
            ...
            if start_date <= date <= end_date:
                ...
                if post.typename == 'GraphSidecar':
                    ...
                    report[date.year].append((date, caption, likes, comments, image_urls))
                else:
                    image_url = post.url
                    report[date.year].append((date, caption, likes, comments, [image_url]))
        except Exception as e:
            print(f"Error processing post: {e}")

    c = canvas.Canvas(pdf_filename, pagesize=letter)
    c.drawString(100, 750, f"Instagram Annual Report")
    ...
    for year, posts in sorted(report.items()):
        if y_position < 100:
            c.showPage()
            y_position = 750
        c.drawString(100, y_position, f"Annual Report for {year}")
        y_position -= 20
        c.drawString(100, y_position, f"Total Posts: {len(posts)}")
        y_position -= 20

        for date, caption, likes, comments, image_urls in posts:
            try:
                if len(image_urls) == 1:
                    ...
                    if y_position - 60 < 0:
                        c.showPage()
                        y_position = 750
                    y_position = draw_wrapped_text(c, caption, 100, y_position - 15, 450)
